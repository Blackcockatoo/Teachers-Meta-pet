...
export function VimanaTetris({
  ...
  const [state, setState] = useState<"running" | "paused" | "gameover">(
    "running",
  );
  ...
  const gridCells = useMemo(() => {
    ...
    for (let y = 0; y < ROWS; y += 1) {
      for (let x = 0; x < COLS; x += 1) {
        ...
        const key = `${x}-${y}`;
        const ghostHere =
          ghostCells.has(`${x},${y}`) && !isActive && !base.filled;
        cells.push(
          <div
            key={key}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/70">
                <div className="px-4 py-3 rounded-xl border border-slate-700 bg-slate-900/90 text-center">
                  ...
                  <div className="mt-3 text-[11px] text-slate-300">
                    ...
                    <span className="font-mono">R</span> to restart
                  </div>
                </div>
              </div>
            )}
