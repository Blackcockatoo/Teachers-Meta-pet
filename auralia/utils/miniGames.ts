...
export function generateFibonacciTrivia(field: Field): TriviaQuestion {
  ...
  const template =
    QUESTION_TEMPLATES[Math.floor(field.prng() * QUESTION_TEMPLATES.length)];
  ...
  // Shuffle options
  const options = [answer, wrong1, wrong2, wrong3].sort(
    () => field.prng() - 0.5,
  );
 ...
 */
export function generateSigilPattern(
  field: Field,
  minLength = 3,
  maxLength = 5,
): number[] {
  const length =
    minLength + Math.floor(field.prng() * (maxLength - minLength + 1));
