...
export function TemporalEchoTrail({
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const wobble =
        Math.sin(timestamp / 620 + curiosity / 35) * (8 + bond / 40);
      const orbit = 18 + (energy / 100) * 26;
      const focusShift = Math.cos(timestamp / 880 + bond / 60) * 10;

      const x =
        size / 2 +
        Math.cos(timestamp / 920 + curiosity / 90) * (orbit + wobble * 0.35);
      const y =
        size / 2 +
        Math.sin(timestamp / 1050 + energy / 70) * (orbit * 0.65) +
        focusShift;

      setEchoes((prev) => {
        const updated = [...prev, { x, y, createdAt: timestamp }];
        const recent = updated.filter(
          (echo) => timestamp - echo.createdAt < MAX_AGE_MS,
        );
  ...
  return (
    ...
    >
      {echoes.map((echo, index) => {
        const age = Math.min(
          1,
          Math.max(0, (currentTime - echo.createdAt) / MAX_AGE_MS),
        );
        const trailOpacity =
          Math.max(0, 0.45 - age * 0.35) * ((index + 1) / MAX_HISTORY);
