...
export function calculateEyeState(
  ...
  if (recentEvent) {
    if (
      recentEvent.type === "sigil_activated" ||
      recentEvent.type === "stat_spike"
    ) {
      // Surprised
      return {
        ...state,
  ...
  if (
    (curiosity > 70 && energy < 30) ||
    (annoyanceLevel > 30 && annoyanceLevel < 80)
  ) {
    return {
      ...state,
      emotion: "confused",
...
export function EyeRenderer({
  ...
  const filterUrl =
    emotion !== "normal" && emotion !== "blinking"
  ...
  if (emotion === "blinking") {
    return (
      <g
        transform={`translate(${eyePos.x + offset.x}, ${eyePos.y + offset.y})`}
      >
        <path
  ...
  const renderEye = (center: { x: number; y: number }, isLeft: boolean) => {
    ...
    return (
      <g
        ...
        transform-origin={`${finalX} ${finalY}`}
      >
        ...
        <circle
          ...
          r={irisSize}
          fill={
            secondaryColor === "rainbow" ? "url(#rainbowGradient)" : irisColor
          }
          filter={filterUrl}
        ...
        >
          {shape === "round" && (
            <animate
              attributeName="r"
              values={`${irisSize};${irisSize + 1};${irisSize}`}
              dur="5s"
              repeatCount="indefinite"
            />
          )}
