...
export const GenomeJewbleRing = memo(function GenomeJewbleRing({
  ...
  const residuesBySequence = useMemo(() => {
    const toResidues = (digits: number[]) =>
      new Set(digits.map((value) => getResidue(value)));
    return {
  ...
  const residueMeta = useMemo(
    () => Array.from({ length: 60 }, (_, index) => getResidueMeta(index)),
    [],
  );
  ...
  return (
    <div
      className={cn(
        "relative mx-auto",
        variant === "dial" ? "max-w-[360px]" : "max-w-[300px]",
      )}
    >
      <svg
        width={size}
