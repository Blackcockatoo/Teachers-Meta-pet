...
export function applyGeneticModulation(
  ...
  const socialMod =
    ((personality.social + personality.affection) / 2 - 50) / 100;
  ...
  return {
    ...
    exploration: clamp(
      baseDrives.exploration * (1 + energyMod * 0.4 + curiosityMod * 0.6),
      0,
      1,
    ),
    connection: clamp(
      baseDrives.connection * (1 + socialMod * 0.5 - independenceMod * 0.3),
      0,
      1,
    ),
    rest: clamp(
      baseDrives.rest * (1 - energyMod * 0.3 - disciplineMod * 0.2),
      0,
      1,
    ),
...
export function refineEmotionalExpression(
  ...
  const { temperament, energy, playfulness, independence, affection } =
    personality;

  // Temperament influences which emotions are more likely
  const temperamentMods: Record<
    string,
    Partial<Record<ExpandedEmotionalState, number>>
  > = {
    Calm: {
      serene: 1.5,
      calm: 1.4,
  ...
  if (emotionWeight < 0.7 && energy > 60 && playfulness > 50 && prng() > 0.4) {
    const activeStates: ExpandedEmotionalState[] = [
      "playful",
      "curious",
      "mischievous",
      "restless",
  ...
  if (emotionWeight < 0.7 && affection > 60 && prng() > 0.5) {
    const connectionStates: ExpandedEmotionalState[] = [
      "affectionate",
      "yearning",
      "protective",
    ];
  ...
  if (emotionWeight < 0.7 && independence > 60 && prng() > 0.5) {
    const solitaryStates: ExpandedEmotionalState[] = [
      "contemplative",
      "serene",
      "withdrawn",
    ];
...
export function consciousnessToResponseContext(
  ...
  return {
    ...
    recentActions: consciousness.memory.actionHistory
      .slice(-5)
      .map((a) => a.action),
...
export function recordExperience(
  ...
  const actionHistory = [
    ...consciousness.memory.actionHistory,
    newMemory,
  ].slice(-100);
  ...
  const personalityDrift = calculatePersonalityDrift(
    actionHistory,
...
function calculatePersonalityDrift(
  ...
  const playCount =
    (actionCounts.get("play") || 0) +
    (actionCounts.get("minigame_victory") || 0);
  if (playCount > 5) {
    drift.playfulness = Math.min(
      baseTraits.playfulness + Math.floor(playCount / 3),
      baseTraits.playfulness + 10,
    );
  }
  ...
  if (feedCount > 5) {
    drift.discipline = Math.min(
      baseTraits.discipline + Math.floor(feedCount / 4),
      baseTraits.discipline + 8,
    );
  }

  // Social interactions increase social
  const socialCount =
    (actionCounts.get("play") || 0) + (actionCounts.get("breeding") || 0);
  if (socialCount > 8) {
    drift.social = Math.min(
      baseTraits.social + Math.floor(socialCount / 4),
      baseTraits.social + 10,
    );
  }

  // Exploration increases curiosity
  const exploreCount =
    (actionCounts.get("exploration_discovery") || 0) +
    (actionCounts.get("exploration_anomaly") || 0);
  if (exploreCount > 3) {
    drift.curiosity = Math.min(
      baseTraits.curiosity + Math.floor(exploreCount / 2),
      baseTraits.curiosity + 12,
    );
  }
...
export function emotionToParticleParams(
  ...
  return {
    particleCount: Math.round(
      baseParams.count * (1 + expressionInfluence * 0.3),
    ),
    ...
    colorIntensity: clamp(
      baseParams.intensity * (0.6 + comfortMod * 0.4),
      0.3,
      1.0,
    ),
