...
export function VimanaMap() {
  ...
  const [selectedId, setSelectedId] = useState<string>(
    vimana.activeCellId ?? vimana.cells[0]?.id ?? "",
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500">
            Scan sacred fields to uncover anomalies and mood boosts.
          </p>
        </div>
        <div className="text-xs text-zinc-400 text-right">
          <p>
            ...
            <span className="text-cyan-300 font-semibold">
              {vimana.scansPerformed}
            </span>
          </p>
          <p>
            Anomalies Resolved:{" "}
            <span className="text-emerald-300 font-semibold">
              {vimana.anomaliesFound}
            </span>
          </p>
        </div>
      </div>
        ...
        {vimana.cells.map((cell) => (
          ...
          >
            <div className="flex items-center justify-between text-sm text-white">
              ...
              <span className="uppercase text-xs text-zinc-300">
                {cell.field}
              </span>
            </div>
            <div className="mt-2 text-xs text-zinc-300 space-y-1">
              ...
              <p>
                Reward:
                <span className="ml-1 capitalize text-emerald-200">
                  {cell.reward}
                </span>
              </p>
            </div>
