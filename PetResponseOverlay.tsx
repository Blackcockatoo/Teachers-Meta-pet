 ...
 */
export function PetResponseOverlay({
  enableAudio = true,
  enableAnticipation = true,
}: PetResponseOverlayProps) {
  ...
  const { currentResponse, isVisible, triggerResponse, responseHistory } =
  ...
  useEffect(() => {
    ...
    const unsubscribe = useStore.subscribe((state, prevState) => {
      // Detect feeding
      if (
        state.vitals.hunger < prevState.vitals.hunger &&
        prevState.vitals.hunger - state.vitals.hunger >= 10
      ) {
      ...
      // Detect playing
      if (
        state.vitals.mood > prevState.vitals.mood &&
      ...
      // Detect cleaning
      if (
        state.vitals.hygiene > prevState.vitals.hygiene &&
        state.vitals.hygiene - prevState.vitals.hygiene >= 10
      ) {
      ...
      if (
        state.vitals.energy > prevState.vitals.energy &&
        state.vitals.energy - prevState.vitals.energy >= 15
      ) {
      ...
      if (state.miniGames && prevState.miniGames) {
        if (
          state.miniGames.memoryHighScore > prevState.miniGames.memoryHighScore
        ) {
          triggerResponse("minigame_victory");
        }
        if (
          state.miniGames.rhythmHighScore > prevState.miniGames.rhythmHighScore
        ) {
          triggerResponse("minigame_victory");
        }
      }
      ...
      if (state.vimana && prevState.vimana) {
        const discoveredCount = state.vimana.cells.filter(
          (c) => c.discovered,
        ).length;
        const prevDiscoveredCount = prevState.vimana.cells.filter(
          (c) => c.discovered,
        ).length;
