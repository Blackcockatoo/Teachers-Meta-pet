...
export function BattleArena() {
  ...
  const handleBattle = () => {
    ...
    if (result === "win") {
      ...
      setLastSummary(
        `Defeat. ${opponent} overpowered the resonance—rest and try again.`,
      );
    }
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500">
            Channel mood and energy into non-violent resonance duels.
          </p>
        </div>
        <div className="text-xs text-zinc-400 text-right">
          ...
          <p>
            Losses:{" "}
            <span className="text-rose-300 font-semibold">{battle.losses}</span>
          </p>
          <p>
            Streak:{" "}
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 space-y-3">
        <div className="flex items-center gap-3 text-sm text-zinc-300">
          ...
          <span>
            Energy Shield Buffer:{" "}
            <strong className="text-cyan-200">
              {Math.round(battle.energyShield)}%
            </strong>
          </span>
        </div>
        <p className="text-xs text-zinc-400 leading-relaxed">
          Shield increases after wins and depletes on losses. Higher shields
          tilt odds in your favor.
        </p>
