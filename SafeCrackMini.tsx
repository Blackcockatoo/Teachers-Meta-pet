      ...
      if (event.key === " " || event.key === "Enter") {
        ...
        setGameState((prev) => {
          ...
          if (inDecoyZone) {
            ...
            setTimeout(() => setWobbleActive(false), 400);
        ...
        <div className="flex-1 flex items-center justify-center">
          ...
          >
            <svg viewBox="0 0 400 400" className="w-full h-full">
              ...
              {/* Current position indicator (needle) with wobble animation */}
              <motion.g
                transform={`rotate(${gameState.currentAngle} 200 200)`}
                animate={
                  wobbleActive
                    ? {
                        rotate: [0, -5, 5, -3, 3, 0],
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/70">
                <div className="px-4 py-3 rounded-xl border border-green-700 bg-slate-900/90 text-center">
                  ...
                  <div className="mt-2 text-xs text-slate-400">
                    Combo: {gameState.combo.n1} → {gameState.combo.n2} →{" "}
                    {gameState.combo.n3}
                  </div>
                  <div className="mt-3 text-[11px] text-slate-300">
                    Press <span className="font-mono">Enter</span> or{" "}
                    <span className="font-mono">R</span> to try again
                  </div>
                </div>
              </div>
            )}
