...
const calculateStats = (rot: number): Stats => {
  ...
  const agilityHarmonic = Math.abs(
    Math.sin((redPhase + bluePhase) * Math.PI * 4),
  );
...
const breedPets = (rot1: number, rot2: number): BreedingResult => {
  ...
  const compatibility = Math.max(
    0,
    Math.round(100 - (harmonicDistance / 30) * 100),
  );
  const avgRotation = Math.round((rot1 + rot2) / 2);

  const primeInfluence = primeRotations.reduce<PrimeRotation>(
    (closest, current) => {
      return minimalDistance(current, avgRotation) <
        minimalDistance(closest, avgRotation)
        ? current
        : closest;
    },
    primeRotations[0],
...
const GenomeWheel: React.FC<GenomeWheelProps> = ({
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {gates.map((gate) => {
        const startAngle =
          (((gate.position + rotation) % 60) / 60) * Math.PI * 2 - Math.PI / 2;
        const endAngle =
          (((gate.position + rotation + 5) % 60) / 60) * Math.PI * 2 -
          Math.PI / 2;
      ...
      {gates.map((gate) => {
        const startAngle =
          ...
          Math.PI / 2;
        const endAngle =
          (((gate.position + rotation + 7) % 60) / 60) * Math.PI * 2 -
          Math.PI / 2;
        const arcRadius = radius - 20;
...
const evolutionStages: EvolutionStage[] = [
  {
    name: "Newborn",
    description:
      "Initial activation of the palindromic wheel. Stats are fluid and reactive.",
  ...
  },
  {
    name: "Primebound",
    description:
      "Prime resonance unlocks rare abilities and mutation potential.",
  ...
  },
  {
    name: "Ascended",
    description:
      "Genome exhibits persistent prime echoes; defense harmonics spike.",
  ...
  },
  {
    ...
    genomeSnapshot:
      "Prime states cycle seamlessly; mutations become intentional.",
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-6 py-10">
        <header className="border-b border-emerald-400/30 pb-6 text-center">
          ...
          <p className="mt-2 text-sm uppercase tracking-[0.4em] text-fuchsia-300">
            Palindromic Wheel Mechanics • Harmonic Interactions • Prime
            Resonance
          </p>
        </header>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    value={rotation}
                    onChange={(event) =>
                      setRotation(Number.parseInt(event.target.value, 10))
                    }
                    disabled={animating}
            ...
            <div className="space-y-6">
              <div className="space-y-4 rounded-2xl border border-emerald-500/40 bg-black/40 p-6">
                ...
                <div className="space-y-4">
                  ...
                  />
                  <StatBar
                    icon={<Zap className="h-4 w-4 text-fuchsia-300" />}
                    label="Agility"
                    value={stats.agility}
                    color="#ff66ff"
                  />
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-mono text-base text-cyan-200">
                        {toHeptaCode(
                          Number.parseInt(selectedGate.palindrome, 10),
                        )}
                      </p>
                    </div>
                  </div>
                  <p className="rounded-md border border-rose-500/40 bg-rose-500/10 p-3 text-xs text-rose-100/80">
                    Gate resonance intensifies when the wheel passes this
                    palindrome while in a prime state, unlocking rare breeding
                    modifiers and defensive harmonics.
                  </p>
                </div>
              )}
              ...
              <div className="space-y-4 rounded-xl border border-emerald-500/30 bg-black/40 p-4">
                ...
                <div className="space-y-3">
                  <input
                    ...
                    value={pet1Rotation}
                    onChange={(event) =>
                      setPet1Rotation(Number.parseInt(event.target.value, 10))
                    }
              ...
              <div className="space-y-4 rounded-xl border border-emerald-500/30 bg-black/40 p-4">
                ...
                <div className="space-y-3">
                  <input
                    ...
                    value={pet2Rotation}
                    onChange={(event) =>
                      setPet2Rotation(Number.parseInt(event.target.value, 10))
                    }
                    className="h-2 w-full cursor-pointer appearance-none rounded bg-emerald-500/20 accent-emerald-400"
                  />
                  <div className="grid grid-cols-2 gap-2 text-xs uppercase tracking-[0.2em] text-white/50">
                ...
                <div className="space-y-3">
                  ...
                  <div className="rounded-md border border-fuchsia-500/40 bg-fuchsia-500/10 p-3 text-xs text-fuchsia-100/80">
                    ...
                    {breedingResult.mutation ? (
                      ...
                      <span className="ml-1 text-fuchsia-100/70">
                  ...
                  <div className="grid gap-2">
                    <StatBar
                      icon={<Heart className="h-4 w-4 text-red-400" />}
                      label="Vitality"
                      value={offspringStats.vitality}
                      color="#ff0044"
                    />
                    <StatBar
                      icon={<Brain className="h-4 w-4 text-sky-300" />}
                      label="Intelligence"
                      value={offspringStats.intelligence}
                      color="#33bbff"
                    />
                    <StatBar
                      icon={<Shield className="h-4 w-4 text-yellow-300" />}
                      label="Defense"
                      value={offspringStats.defense}
                      color="#ffee66"
                    />
                    <StatBar
                      icon={<Zap className="h-4 w-4 text-fuchsia-300" />}
                      label="Agility"
                      value={offspringStats.agility}
                      color="#ff66ff"
                    />
                  </div>
                </div>
          ...
          <section className="space-y-8 rounded-2xl border border-emerald-500/40 bg-black/40 p-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {evolutionStages.map((stage, index) => {
                ...
                const primeAligned =
                  stage.requiresPrime && isNearPrime(rotation);
                return (
                  <div
                    key={stage.name}
